...
**get()**: процесс вызывает `propose(v)` с текущим известным значением $v$ (no-op-предложение) и получает результат через возврат из операции propose:
1. Если сам набрал кворум ACK без NACK -> return bufferedValue;
2. Если узнал значение через чужой [LEARN, v'] -> return v' .

Это гарантирует синхронизацию со всеми инкрементами, произошедшими до начала чтения, и линеаризуемость.
...
Берём инфу из 4.3.1 и используем оттуда определение с лексикографическим порядком и способом обновить или прочитать значение с помощью propose((t, v)) и propose((0 $v_0$)) соответственно.

**write(val)**: процесс берёт временную метку t (увеличивает свой локальный счётчик меток, как в ABD при инкременте t у writer'а) и вызывает propose((t, val)):
- если сам набрал кворум ACK без NACK -> return bufferedValue (своя же пара 
(t,val));
- если узнал через чужой [LEARN, v'] -> return v'.

**read()**: процесс вызывает propose((0, $v_0$)) — с самой младшей возможной парой и получает результат тем же путём (через return bufferedValue либо через адаптацию LEARN). Из полученной пары (t,val) возвращается val.

И получается, что $v_0$ зависит от того, какой тип данных мы выбрали для $v$, т.е. для числа это мог бы быть 0 или флаг типа "-inf", будь это строка мы бы взяли null/None.
